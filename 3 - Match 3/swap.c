// Stage 3: Swap demo for CS50 Match-3 (Raylib).
// - Creates an 8x8 board with colored tiles
// - Renders static board (no input/swaps/matches yet)
// - Keeps your RenderTexture-based virtual resolution
//
// Assets: none required for this stage (tiles are simple rectangles)

#include <stdlib.h>
#include <stdio.h>
#include <time.h>
#include <math.h>

#include "raylib.h"

#define TWEEN_IMPL
#include "tween.h"

// ----------------- Virtual resolution & window -----------------
static const int VIRTUAL_W = 384;
static const int VIRTUAL_H = 216;

static const int WINDOW_W  = 1280;
static const int WINDOW_H  = 720;

static RenderTexture2D target;

// ----------------- Board configuration -----------------
#define BOARD_ROWS 8
#define BOARD_COLS 8
#define TILE_SIZE  32

// Visual offset of the board inside the virtual screen
typedef struct {
    int x;
    int y;
} Point;

static Point boardOrigin; // top-left pixel where board is drawn

// Number of distinct colors and varieties for tiles (simple integers for now)
#define COLOR_COUNT   6   // tweak freely (3..8 recommended)
#define VARIETY_COUNT 3   // small variety to start

typedef struct {
    int gridX;   // 1..BOARD_COLS (logical grid coordinate)
    int gridY;   // 1..BOARD_ROWS
    int color;   // 1..COLOR_COUNT
    int variety; // 1..VARIETY_COUNT
    float x;     // pixel position (top-left), derived from origin + gridX*size
    float y;
} Tile;

typedef struct {
    Tile tiles[BOARD_ROWS][BOARD_COLS];
} Board;

static Board board;

// ----------------- Utility -----------------
static inline Color ColorFromIndex(int idx) {
    // Return a stable palette for 1..COLOR_COUNT
    // Choose high-contrast colors for clarity
    switch (idx) {
        case 1: return (Color){231,  76,  60, 255}; // red
        case 2: return (Color){ 46, 204, 113, 255}; // green
        case 3: return (Color){ 52, 152, 219, 255}; // blue
        case 4: return (Color){241, 196,  15, 255}; // yellow
        case 5: return (Color){155,  89, 182, 255}; // purple
        case 6: return (Color){230, 126,  34, 255}; // orange
        default: return (Color){200, 200, 200, 255};
    }
}

static inline Rectangle TileRectPx(int gx, int gy) {
    // Convert 1-based grid coords into pixel rectangle relative to origin
    float x = (float)boardOrigin.x + (gx - 1) * TILE_SIZE;
    float y = (float)boardOrigin.y + (gy - 1) * TILE_SIZE;
    return (Rectangle){ x, y, (float)TILE_SIZE, (float)TILE_SIZE };
}

// ----------------- Board init & draw -----------------
static void BoardInit(void) {
    // Center the 8x8 grid in the virtual canvas
    const int w = BOARD_COLS * TILE_SIZE;
    const int h = BOARD_ROWS * TILE_SIZE;
    boardOrigin.x = (VIRTUAL_W - w) / 2;
    boardOrigin.y = (VIRTUAL_H - h) / 2;

    // Random seed
    srand((unsigned)time(NULL));

    for (int gy = 1; gy <= BOARD_ROWS; ++gy) {
        for (int gx = 1; gx <= BOARD_COLS; ++gx) {
            Tile *t = &board.tiles[gy - 1][gx - 1];
            t->gridX = gx;
            t->gridY = gy;
            t->color = 1 + (rand() % COLOR_COUNT);
            t->variety = 1 + (rand() % VARIETY_COUNT);

            Rectangle r = TileRectPx(gx, gy);
            t->x = r.x;
            t->y = r.y;
        }
    }
}

static void DrawBoardBackdrop(void) {
    // Panel behind the board for contrast
    const int w = BOARD_COLS * TILE_SIZE;
    const int h = BOARD_ROWS * TILE_SIZE;

    Rectangle outer = { (float)boardOrigin.x - 4, (float)boardOrigin.y - 4,
                        (float)w + 8, (float)h + 8 };
    DrawRectangleRec(outer, (Color){30, 30, 35, 255}); // dark frame

    Rectangle inner = { (float)boardOrigin.x, (float)boardOrigin.y,
                        (float)w, (float)h };
    DrawRectangleRec(inner, (Color){15, 15, 20, 255}); // board background
}

static void DrawTile(const Tile *t) {
    // Base rectangle
    Rectangle r = { t->x, t->y, (float)TILE_SIZE, (float)TILE_SIZE };

    // Slight drop shadow for depth
    DrawRectangle((int)r.x + 1, (int)r.y + 2, TILE_SIZE, TILE_SIZE, (Color){0,0,0,40});

    // Fill
    Color c = ColorFromIndex(t->color);
    DrawRectangle((int)r.x, (int)r.y, TILE_SIZE, TILE_SIZE, c);

    // Outline
    DrawRectangleLinesEx(r, 1.0f, (Color){255,255,255,40});

    // Variety label (small)
    // Use a tiny font size to avoid clutter; adjust if your font is different
    char buf[8];
    // Print like "v1", "v2", ...
    snprintf(buf, sizeof(buf), "v%d", t->variety);
    DrawText(buf, (int)r.x + 4, (int)r.y + 4, 8, (Color){255,255,255,200});
}

static void BoardDraw(void) {
    DrawBoardBackdrop();
    // Optional grid lines (subtle)
    for (int i = 0; i <= BOARD_COLS; ++i) {
        int x = boardOrigin.x + i * TILE_SIZE;
        DrawLine(x, boardOrigin.y, x, boardOrigin.y + BOARD_ROWS * TILE_SIZE, (Color){255,255,255,15});
    }
    for (int j = 0; j <= BOARD_ROWS; ++j) {
        int y = boardOrigin.y + j * TILE_SIZE;
        DrawLine(boardOrigin.x, y, boardOrigin.x + BOARD_COLS * TILE_SIZE, y, (Color){255,255,255,15});
    }
    // Tiles
    for (int gy = 1; gy <= BOARD_ROWS; ++gy) {
        for (int gx = 1; gx <= BOARD_COLS; ++gx) {
            DrawTile(&board.tiles[gy - 1][gx - 1]);
        }
    }
}

// ----------------- App lifecycle -----------------
static void InitApp(void) {
    // Nothing tweened in swap0; init system anyway for continuity
    Tween_InitSystem();
    BoardInit();
}

int main(void) {
    SetConfigFlags(FLAG_VSYNC_HINT | FLAG_WINDOW_RESIZABLE);
    InitWindow(WINDOW_W, WINDOW_H, "Match-3 — swap0: Just a Board");
    SetTargetFPS(60);

    target = LoadRenderTexture(VIRTUAL_W, VIRTUAL_H);
    SetTextureFilter(target.texture, TEXTURE_FILTER_POINT);

    InitApp();

    while (!WindowShouldClose()) {
        const float dt = GetFrameTime();
        (void)dt; // reserved for next stages
        Tween_UpdateAll(dt);

        // --- Draw to virtual backbuffer ---
        BeginTextureMode(target);
            ClearBackground((Color){18, 20, 24, 255});

            // Title
            DrawText("swap0 — Just a Board", 6, 6, 8, (Color){200,225,255,255});

            // Board
            BoardDraw();
        EndTextureMode();

        // --- Present with scaling ---
        BeginDrawing();
            ClearBackground(BLACK);
            float scale = fminf(
                (float)GetScreenWidth()  / (float)VIRTUAL_W,
                (float)GetScreenHeight() / (float)VIRTUAL_H
            );
            Rectangle src = { 0, 0, (float)target.texture.width, -(float)target.texture.height };
            Rectangle dst = {
                (GetScreenWidth()  - (float)VIRTUAL_W * scale) * 0.5f,
                (GetScreenHeight() - (float)VIRTUAL_H * scale) * 0.5f,
                (float)VIRTUAL_W * scale,
                (float)VIRTUAL_H * scale
            };
            DrawTexturePro(target.texture, src, dst, (Vector2){0,0}, 0.0f, WHITE);
        EndDrawing();
    }

    UnloadRenderTexture(target);
    CloseWindow();
    return 0;
}
